...
[YOLOv7 object detection architecture](https://github.com/WongKinYiu/yolov7) to detect wind turbines in raw satellite imagery.
...
## Project Organization
    ...
    |            └── train                                       <- Training data split (empty due to upload constraints).
    |                  └── labels
    |                  └── images
    |            └── test                                        <- Test data split (empty due to upload constraints).
    |                  └── labels
    |                  └── images
    |            └── valid                                       <- Validation data split (empty due to upload constraints).
    |                  └── labels
    |                  └── images
